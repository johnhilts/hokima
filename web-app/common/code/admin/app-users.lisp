...
(defun add-external-user (user-name user-email user-fingerprint)
  ...
  (let ((secure-user (make-instance 'user:application-secure-user :user-login user-email :user-password "password-not-set" :user-fingerprint user-fingerprint :user-id ""))
        (web-user (make-external-app-user user-name user-email user-fingerprint)))
